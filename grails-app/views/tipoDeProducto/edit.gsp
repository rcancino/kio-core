...
<head>
	...
	<title>Tipo de Producto ${tipoDeProductoInstance}</title>
</head>
<body>

	<content tag="header">
		<h3>Tipo de Producto ${tipoDeProductoInstance.id}</h3>
	</content>
	<content tag="form">
		...
		<g:form class="form-horizontal" action="update" method="PUT">
			<g:hiddenField name="version" value="${tipoDeProductoInstance?.version}" />
			<fieldset>
				<legend>Tipo de Producto</legend>
				<f:with bean="${tipoDeProductoInstance}">
					<g:hiddenField name="id" value="${tipoDeProductoInstance?.id}" />
